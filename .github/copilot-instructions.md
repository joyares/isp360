...
- Dropdown fields: MUST use `<select class="form-select">` with a `disabled selected` empty placeholder option.
- Table action column position: In all list tables, the `Action` column MUST be the first (leftmost) column.
- Table action button style: Use lightweight icon buttons with `btn btn-link p-0`, tooltip attributes (`data-bs-toggle="tooltip"`, `data-bs-placement="top"`), and clear labels like Edit/Off.
