...
// Resolve the URL prefix that points to the public/ directory.
if (!isset($appBasePath)) {
    $documentRootPath = isset($_SERVER['DOCUMENT_ROOT']) ? realpath($_SERVER['DOCUMENT_ROOT']) : false;
    $publicRootPath = realpath(dirname(__DIR__, 2) . DIRECTORY_SEPARATOR . 'public');

    $appBasePath = '';
    if ($documentRootPath && $publicRootPath) {
        $normalizedDocumentRoot = rtrim(str_replace('\\', '/', (string) $documentRootPath), '/');
        $normalizedPublicRoot = rtrim(str_replace('\\', '/', $publicRootPath), '/');
...
$pageTitle = isset($pageTitle) && $pageTitle !== '' ? htmlspecialchars($pageTitle, ENT_QUOTES, 'UTF-8') : 'ISP360';
?>
<!DOCTYPE html>
