...
if ($search !== '') {
  $where[] = '(t.ticket_no LIKE :search1 OR c.username LIKE :search2 OR c.phone_no LIKE :search3 OR t.issue_details LIKE :search4)';
...
<div class="row gx-3">
      ...
      <div class="px-2 py-2 pb-2 pt-2">
        ...
        <div class="row gy-2 mb-3">
          ...
          <div class="col-auto ms-auto text-end">
            <a class="btn btn-falcon-default btn-sm" href="<?= $appBasePath ?>/app/support-desk/add-ticket.php"><span
        ...
        <!-- Row 2: Filters -->
        <form id="filterForm" method="get" action="<?= $appBasePath ?>/app/support-desk/all-tickets.php"
          class="row g-2 mb-3">
          <div class="col">
            <div class="input-group input-group-sm">
              <input class="form-control form-control-sm shadow-none" type="search" name="q"
                value="<?= htmlspecialchars($search) ?>" placeholder="Search by ticket, customer, phone or issue"
                aria-label="search" />
              <button class="btn btn-sm btn-outline-secondary border-300 hover-border-secondary" type="submit"><span
                  class="fa fa-search fs-10"></span></button>
            </div>
          </div>
          <div class="col-auto d-flex justify-content-end gap-2">
            <input type="hidden" id="filterFromDate" name="filter_from_date"
            ...
            <input class="form-control mb-5 datetimepicker" id="supportDeskDateRangePicker" type="text"
              placeholder="dd/mm/yyyy to dd/mm/yyyy"
              value="<?= (new DateTime($filterFromDate))->format('d/m/Y') ?> to <?= (new DateTime($filterToDate))->format('d/m/Y') ?>"
              data-options='{"mode":"range","dateFormat":"d/m/Y","disableMobile":true,"position":"below","predefinedRanges":true}' />
            ...
            <script>
              (function () {
                window.addEventListener('DOMContentLoaded', function () {
                  ...
                  // Allow theme.js to initialize flatpickr first
                  setTimeout(function () {
                    if (picker && picker._flatpickr) {
                      picker._flatpickr.config.onChange.push(function (selectedDates) {
            ...
            ?>
            <div class="dropdown">
              <button class="btn btn-primary btn-sm dropdown-toggle py-1 px-2" type="button" id="filterStatusDropdown"
              ...
              <ul class="dropdown-menu" aria-labelledby="filterStatusDropdown">
                ...
                ?>
                <li>
                  <a class="dropdown-item <?= $showAllStatuses ? 'active' : '' ?>"
                    href="<?= $appBasePath ?>/app/support-desk/all-tickets.php?<?= http_build_query($allStatusQuery) ?>">All</a>
                </li>
                <li>
            ...
            ?>
            <div class="dropdown">
              <button class="btn btn-primary btn-sm dropdown-toggle py-1 px-2" type="button" id="filterPriorityDropdown"
                data-bs-toggle="dropdown" aria-expanded="false" style="font-size:0.75rem;">
                <?= $priorityLabel ?>
              </button>
              <ul class="dropdown-menu" aria-labelledby="filterPriorityDropdown">
                <li>
                  <a class="dropdown-item <?= $filterPriorityId === 0 ? 'active' : '' ?>"
                    href="<?= $appBasePath ?>/app/support-desk/all-tickets.php?<?= http_build_query(array_merge($_GET, ['filter_priority' => 0])) ?>">Priority</a>
                </li>
            ...
            ?>
            <div class="dropdown">
              <button class="btn btn-primary btn-sm dropdown-toggle py-1 px-2" type="button" id="filterAssignedDropdown"
                data-bs-toggle="dropdown" aria-expanded="false" style="font-size:0.75rem;">
                <?= $assignedLabel ?>
              </button>
              <ul class="dropdown-menu" aria-labelledby="filterAssignedDropdown">
                <li>
                  <a class="dropdown-item <?= $filterAssigned === 0 ? 'active' : '' ?>"
                    href="<?= $appBasePath ?>/app/support-desk/all-tickets.php?<?= http_build_query(array_merge($_GET, ['filter_assigned' => 0])) ?>">Agent</a>
                </li>
            ...
            ?>
            <div class="dropdown">
              <button class="btn btn-primary btn-sm dropdown-toggle py-1 px-2" type="button" id="filterBranchDropdown"
                data-bs-toggle="dropdown" aria-expanded="false" style="font-size:0.75rem;">
                <?= $branchLabel ?>
              </button>
              <ul class="dropdown-menu" aria-labelledby="filterBranchDropdown">
                <li>
                  <a class="dropdown-item <?= $filterBranchId === 0 ? 'active' : '' ?>"
                    href="<?= $appBasePath ?>/app/support-desk/all-tickets.php?<?= http_build_query(array_merge($_GET, ['filter_branch' => 0])) ?>">Branch</a>
                </li>
            ...
            <a class="btn btn-link p-0 text-primary" href="<?= $appBasePath ?>/app/support-desk/all-tickets.php"
            ...
            <tbody>
              ...
              <?php else: ?>
                <?php foreach ($tickets as $ticket): ?>
                  ...
                  ?>
                  <tr class="table-row-hover <?= $selectedTicketId === (int) $ticket['ticket_id'] ? 'active-row' : '' ?>"
                    style="cursor:pointer;"
                    onclick="window.location='<?= $appBasePath ?>/app/support-desk/all-tickets.php<?= htmlspecialchars($ticketDetailsQuery, ENT_QUOTES, 'UTF-8') ?>'">
                    <td class="align-middle ps-2 ticket-row-action" onclick="event.stopPropagation();">
                      <a class="btn btn-link p-0"
                        href="<?= $appBasePath ?>/app/support-desk/all-tickets.php<?= htmlspecialchars($ticketDetailsQuery) ?>"
                        data-bs-toggle="tooltip" data-bs-placement="top" title="Edit" onclick="event.stopPropagation();">
                        <span class="fas fa-edit text-500"></span>
                      </a>
                      <?php if ((int) ($ticket['customer_id'] ?? 0) > 0): ?>
                        <a class="btn btn-link p-0 ms-2 text-primary"
                          href="<?= $appBasePath ?>/app/support-desk/customer-details.php?id=<?= (int) $ticket['customer_id'] ?>"
                          data-bs-toggle="tooltip" data-bs-placement="top" title="View Customer"
                    ...
                    <td class="align-middle ps-2">
                      <div class="d-flex flex-column gap-1">
                        ...
                        <small
                          class="badge rounded badge-subtle-<?= $statusClass ?> align-self-start"><?= htmlspecialchars($statusLabel) ?></small>
                        <div class="d-flex align-items-center gap-1">
                          <small class="badge badge-subtle-success rounded-pill" style="font-size: 0.7rem;"><span
                              class="fas fa-hourglass-end me-1"
                              style="font-size: 0.65rem;"></span><?= $durationText ?></small>
                          <small class="badge badge-subtle-secondary rounded-pill" style="font-size: 0.7rem;"><span
                              class="fas fa-sync me-1"
                              style="font-size: 0.65rem;"></span><?= $updatedTime->format('H:i') ?></small>
                        </div>
                      </div>
                    </td>
                      ...
                      <div class="d-flex align-items-center gap-2 position-relative">
                        <div class="avatar avatar-xl">
                          <?php if ($companyLogoUrl !== ''): ?>
                            <img class="rounded-circle" src="<?= htmlspecialchars($companyLogoUrl) ?>"
                              alt="<?= htmlspecialchars($companyLabel) ?> logo"
                              style="object-fit: cover; width: 100%; height: 100%;">
                          <?php else: ?>
                            <div class="avatar-name rounded-circle">
                              <span><?= htmlspecialchars(strtoupper(substr($companyLabel !== '-' ? $companyLabel : $clientLabel, 0, 1))) ?></span>
                            </div>
                          <?php endif; ?>
                        </div>
                        <div class="d-flex flex-column gap-1">
                          <h6 class="mb-0"><a class="stretched-link text-900"
                              href="<?= $appBasePath ?>/app/support-desk/all-tickets.php<?= htmlspecialchars($ticketDetailsQuery) ?>"><?= htmlspecialchars($clientLabel) ?></a>
                          </h6>
                          <small
                            class="text-600"><?= htmlspecialchars((string) (($ticket['customer_phone'] ?? '') !== '' ? $ticket['customer_phone'] : '-')) ?></small>
                        </div>
                      </div>
                    </td>
                    <td class="align-middle subject py-2 pe-4">
                      <div class="d-flex flex-column gap-1">
                        <small
                          class="badge badge-subtle-primary rounded-pill text-start align-self-start"><?= htmlspecialchars((string) ($ticket['category_name'] ?: 'Uncategorized')) ?></small>
                    ...
                    <td class="align-middle priority pe-4">
                      <div class="d-flex align-items-center gap-2 ms-md-4 ms-xl-0" style="width:7.5rem;">
                        <div style="--falcon-circle-progress-bar:100"><svg class="circle-progress-svg" width="26"
                            ...
                            <circle class="progress-bar-top" cx="60" cy="60" r="54" fill="none" stroke-linecap="round"
                              stroke="<?= htmlspecialchars($priorityStroke) ?>" stroke-width="12"></circle>
                          </svg></div>
  ...
  <div class="col-xl-3 col-lg-3 col-12" id="selected-ticket-details">
        ...
        <div class="row h-100 justify-content-between g-0">
              ...
              <?php foreach ($statusSummary as $sr):
                ...
                <div class="d-flex align-items-center gap-1 mb-1">
                  <div class="d-flex align-items-center gap-2 min-w-0">
                    ...
                    <span class="fw-semi-bold text-truncate"
                      style="max-width:8.5rem;"><?= htmlspecialchars($sLabel) ?></span>
                  </div>
          ...
          <div class="col-auto position-relative d-flex align-items-center align-self-start" style="top: 0;">
            <div class="position-relative" style="width:106px; height:106px;">
              <canvas id="ticketsSummaryCanvas" width="106" height="106"
                style="position: absolute; left: 0px; top: 0px; width: 106px; height: 106px; user-select: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0); padding: 0px; margin: 0px; border-width: 0px;"
              ...
              <div class="position-absolute top-50 start-50 translate-middle text-1100 fs-7 fw-semi-bold">
                <?= $totalTickets ?>
              </div>
            </div>
          </div>
    ...
    <?php if ($selectedTicket !== null): ?>
      ...
      <div class="card mb-3"
        ...
        <div class="card-body position-relative">
          ...
          <div class="d-flex align-items-center justify-content-between gap-2 mb-2">
            <div class="d-flex align-items-center gap-2 min-w-0">
              ...
              <span class="badge badge-subtle-success rounded-pill"><span
                  class="fas fa-calendar-plus me-1"></span><?= $durationText ?></span>
            </div>
            <div class="d-flex align-items-center gap-2">
              <button class="btn btn-link p-0 text-primary" type="button" title="Print 58mm" aria-label="Print 58mm"
                onclick="printThermalTicket()">
                <span class="fas fa-print fs-9"></span>
              </button>
              <button class="btn btn-link p-0 text-primary" type="button" data-bs-toggle="modal"
                data-bs-target="#editTicketModal" title="Edit Details" aria-label="Edit Details">
          ...
          <div class="d-flex align-items-center gap-2 mb-2">
            ...
            <button class="btn btn-link p-0 text-primary" type="button" data-bs-toggle="tooltip" data-bs-placement="top"
              title="Copy Ticket ID"
              onclick="copyToClipboard('<?= htmlspecialchars((string) $selectedTicket['ticket_no']) ?>')">
          ...
          <div class="d-flex align-items-center gap-2 mb-3">
            <small class="badge badge-subtle-info rounded-pill"><span
                class="fas fa-clock me-1"></span><?= $createdTime->format('M d, Y H:i') ?></small>
            <small class="badge badge-subtle-secondary rounded-pill"><span
                class="fas fa-sync me-1"></span><?= $updatedTime->format('M d, Y H:i') ?></small>
          </div>
            ...
            <div class="d-flex align-items-center gap-2">
              <div class="flex-fill min-w-0">
                <select class="form-select form-select-sm" name="priority_id" aria-label="Update priority">
                  <option value="0" disabled <?= (int) ($selectedTicket['priority_id'] ?? 0) <= 0 ? 'selected' : '' ?>>
                    Priority</option>
                  <?php foreach ($priorities as $item): ?>
                    <option value="<?= (int) $item['priority_id'] ?>" <?= (int) $selectedTicket['priority_id'] === (int) $item['priority_id'] ? 'selected' : '' ?>><?= htmlspecialchars((string) $item['priority_name']) ?>
                    </option>
                  <?php endforeach; ?>
                </select>
              </div>
              <div class="flex-fill min-w-0">
                <select class="form-select form-select-sm" name="assigned_employee_id" aria-label="Update assigned agent">
                  <option value="0" <?= (int) ($selectedTicket['assigned_employee_id'] ?? 0) <= 0 ? 'selected' : '' ?>>Agent
                  </option>
                  <?php foreach ($employees as $item): ?>
                    <?php
                    ...
                    ?>
                    <option value="<?= (int) $item['admin_user_id'] ?>" <?= (int) $selectedTicket['assigned_employee_id'] === (int) $item['admin_user_id'] ? 'selected' : '' ?>>
              ...
              <div class="flex-shrink-0">
                <button class="btn btn-falcon-default btn-sm" type="submit" data-bs-toggle="tooltip"
                  data-bs-placement="top" title="Update ticket">
      ...
      <div class="card mb-3"
        style="background-image: url(<?= $appBasePath ?>/assets/img/icons/spot-illustrations/corner-2.png); background-position: right bottom; background-repeat: no-repeat; background-size: auto 100%;">
        <div class="card-body position-relative">
          <div class="d-flex align-items-center justify-content-between gap-2 mb-2">
            <h6 class="mb-0">Customer<span class="badge badge-subtle-info rounded-pill ms-2">Active</span></h6>
            <a class="btn btn-link p-0 text-primary" href="#" data-bs-toggle="tooltip" data-bs-placement="top"
              title="View Customer" aria-label="View Customer">
              <span class="fas fa-eye fs-9"></span>
            </a>
          </div>
          ...
          <?php if ($customerPhone !== ''): ?>
            <div class="d-flex align-items-center flex-wrap gap-3 mb-1">
              <div class="d-flex align-items-center gap-2 min-w-0">
                ...
                <button class="btn btn-link p-0 text-primary" type="button" data-bs-toggle="tooltip" data-bs-placement="top"
                  title="Copy Name" onclick="copyToClipboard('<?= htmlspecialchars($customerName) ?>')">
                  <span class="fas fa-copy fs-9"></span>
                </button>
              </div>
              <div class="d-flex align-items-center gap-2 min-w-0">
                ...
                <button class="btn btn-link p-0 text-primary" type="button" data-bs-toggle="tooltip" data-bs-placement="top"
                  title="Copy Phone" onclick="copyToClipboard('<?= htmlspecialchars($customerPhone) ?>')">
                  <span class="fas fa-copy fs-9"></span>
                </button>
              </div>
            </div>
        ...
        <div class="card-body position-relative">
          ...
          <form method="post"
            ...
            <div class="d-flex align-items-start gap-2">
              ...
              <button class="btn btn-falcon-default btn-sm" type="submit" data-bs-toggle="tooltip" data-bs-placement="top"
                title="Add note">
              ...
              <div class="modal-body">
                ...
                <div class="mb-3">
                  ...
                  <select class="form-select" id="categoryId" name="category_id">
                    ...
                    <?php foreach ($categories as $item): ?>
                      <option value="<?= (int) $item['category_id'] ?>" <?= (int) $selectedTicket['category_id'] === (int) $item['category_id'] ? 'selected' : '' ?>><?= htmlspecialchars((string) $item['category_name']) ?>
                      </option>
                    <?php endforeach; ?>
                  </select>
                </div>
                ...
                <div class="mb-3">
                  ...
                  <select class="form-select" id="statusId" name="ticket_status_id">
                    ...
                    <?php foreach ($ticketStatuses as $item): ?>
                      <option value="<?= (int) $item['ticket_status_id'] ?>" <?= (int) $selectedTicket['ticket_status_id'] === (int) $item['ticket_status_id'] ? 'selected' : '' ?>>
                        <?= htmlspecialchars((string) $item['status_name']) ?>
                      </option>
                    <?php endforeach; ?>
                  </select>
                ...
                <div class="mb-3">
                  ...
                  <textarea class="form-control" id="issueDetails" name="issue_details" rows="3"
                    required><?= htmlspecialchars((string) $selectedTicket['issue_details']) ?></textarea>
                </div>
