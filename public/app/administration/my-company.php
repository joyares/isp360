...
$pdo->exec(
  ...
  $legacySafeColumns = [
    ...
    'contact_person_img' => "ALTER TABLE mycompany ADD COLUMN contact_person_img VARCHAR(255) NULL",
  ...
  if ($ispts_has_column($pdo, 'mycompany', 'avatar') && $ispts_has_column($pdo, 'mycompany', 'contact_person_img')) {
    $pdo->exec('UPDATE mycompany SET contact_person_img = COALESCE(NULLIF(contact_person_img, ""), avatar)');
  }
  ...
  $partnerContactImageColumn = $ispts_has_column($pdo, 'mycompany', 'contact_person_img')
    ? 'contact_person_img'
    : ($ispts_has_column($pdo, 'mycompany', 'avatar') ? 'avatar' : '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($alert === null) {
            ...
            $insertStmt = $pdo->prepare(
                'INSERT INTO mycompany (
                  username, password, email, phone, company, logo_icon, logo_main, contact_person_img,
    ...
    if ($action === 'save_partner') {
        ...
        if (!$partnersTableExists) {
          $alert = ['type' => 'danger', 'message' => 'Partners table not found.'];
        } else {
          $partnersHasColumn = static function (string $column) use ($ispts_has_column, $pdo): bool {
            return $ispts_has_column($pdo, 'partners', $column);
          };

          $partnersLegacySafeColumns = [
            'logo_icon' => "ALTER TABLE partners ADD COLUMN logo_icon VARCHAR(255) NULL",
            'logo_main' => "ALTER TABLE partners ADD COLUMN logo_main VARCHAR(255) NULL",
            'contact_person_img' => "ALTER TABLE partners ADD COLUMN contact_person_img VARCHAR(255) NULL",
            'contact_person_name' => "ALTER TABLE partners ADD COLUMN contact_person_name VARCHAR(200) NULL",
            'contact_person_phone' => "ALTER TABLE partners ADD COLUMN contact_person_phone VARCHAR(200) NULL",
            'contact_person_alt_phone' => "ALTER TABLE partners ADD COLUMN contact_person_alt_phone VARCHAR(200) NULL",
            'contact_person_email' => "ALTER TABLE partners ADD COLUMN contact_person_email VARCHAR(300) NULL",
          ];
          foreach ($partnersLegacySafeColumns as $column => $sql) {
            if (!$partnersHasColumn($column)) {
              $pdo->exec($sql);
            }
          }
          ...
          if ($alert === null) {
            ...
            if ($incomingId > 0) {
              ...
              if (!empty($setParts)) {
                $sql = 'UPDATE partners SET ' . implode(', ', $setParts) . ' WHERE id = :id' . $whereDeleted;
                ...
                $alert = ['type' => 'danger', 'message' => 'Partner update failed: no writable columns found in partners table.'];
              }
            } else {
              $insertColumns = [];
              $insertPlaceholders = [];
              ...
              if (!empty($insertColumns)) {
                $sql = 'INSERT INTO partners (' . implode(', ', $insertColumns) . ') VALUES (' . implode(', ', $insertPlaceholders) . ')';
                $stmt = $pdo->prepare($sql);
...
$showCompanyEditForm = $activeTab === 'company' && (int) ($formData['id'] ?? 0) > 0;
...
<div class="row gx-3 gy-3">
      ...
      <div class="card-header bg-body-tertiary d-flex flex-between-center py-2">
        ...
        <div class="dropdown font-sans-serif position-static d-inline-block btn-reveal-trigger">
          <?php if ($motherCompanyEnabled !== null): ?>
            <small class="badge rounded <?= $motherCompanyEnabled === 1 ? 'badge-subtle-success' : 'badge-subtle-danger' ?> me-2">
          ...
          <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-mother-company">
            <a class="dropdown-item" href="<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/app/administration/my-company.php?tab=company#partner-form">View</a>
            <a class="dropdown-item" href="<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/app/administration/my-company.php?tab=company<?= $motherCompanyId > 0 ? '&edit_id=' . $motherCompanyId : '' ?>#partner-form">Edit</a>
            <div class="dropdown-divider"></div>
            <a class="dropdown-item text-danger" href="<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/app/administration/my-company.php?tab=company">Refresh</a>
          </div>
        </div>
      </div>
      <div class="card-body">
      ...
      <div class="card-header bg-body-tertiary d-flex flex-between-center py-2">
        ...
        <div class="dropdown font-sans-serif position-static d-inline-block btn-reveal-trigger">
          <?php if ($partnerHeaderEnabled !== null): ?>
            <small class="badge rounded <?= $partnerHeaderEnabled === 1 ? 'badge-subtle-success' : 'badge-subtle-danger' ?> me-2">
              <?= $partnerHeaderEnabled === 1 ? 'Active' : 'Inactive' ?>
            </small>
          <?php endif; ?>
          <button class="btn btn-link text-600 btn-sm dropdown-toggle btn-reveal dropdown-caret-none" type="button" id="dropdown-partners" data-bs-toggle="dropdown" data-boundary="window" aria-haspopup="true" aria-expanded="false" data-bs-reference="parent">
            <span class="fas fa-ellipsis-h fs-10"></span>
          </button>
          <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="dropdown-partners">
            <a class="dropdown-item" href="<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/app/administration/my-company.php?tab=partners#partner-tab-form">View</a>
            <a class="dropdown-item" href="<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/app/administration/my-company.php?tab=partners<?= $partnerListEditId > 0 ? '&partner_edit_id=' . $partnerListEditId : '' ?>#partner-tab-form">Edit</a>
            <div class="dropdown-divider"></div>
            <a class="dropdown-item text-danger" href="<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/app/administration/my-company.php?tab=partners">Refresh</a>
          </div>
        </div>
      </div>
      <div class="card-body">
        <div class="row g-3 h-100">
          <?php if (empty($partnerTabRows)): ?>
            <div class="col-12">
              <div class="text-center py-3 text-600">No partners found.</div>
            </div>
          <?php else: ?>
            <?php foreach ($partnerTabRows as $pr): ?>
              ...
              <div class="col-sm-6 col-lg-12" data-partner-card-id="<?= $partnerRowId ?>">
                <div class="card position-relative rounded-4">
                  <div class="bg-holder bg-card rounded-4" style="background-image:url(<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/assets/img/icons/spot-illustrations/corner-1.png);"></div>
                  <div class="card-body p-3 pt-4 pt-xxl-4">
                    <div class="row mb-2">
                      <div class="col text-end d-flex justify-content-end">
                        <?php if ($partnerMainLogoUrl !== ''): ?>
                          <img class="js-partner-main-logo" src="<?= htmlspecialchars($partnerMainLogoUrl, ENT_QUOTES, 'UTF-8') ?>" alt="Main logo" height="72" style="max-width: 240px; object-fit: contain;">
                        <?php else: ?>
                    ...
                    <div class="row align-items-stretch">
                      ...
                      <div class="col-6 text-start">
                        <div class="fs-10 text-600 mb-1"><strong>Contact Person</strong></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars((string) (($pr['contact_person_name'] ?? '') !== '' ? $pr['contact_person_name'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars((string) (($pr['contact_person_phone'] ?? '') !== '' ? $pr['contact_person_phone'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars((string) (($pr['contact_person_alt_phone'] ?? '') !== '' ? $pr['contact_person_alt_phone'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars((string) (($pr['contact_person_email'] ?? '') !== '' ? $pr['contact_person_email'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                      </div>
                      <div class="col-6 text-end">
                        <h6 class="text-primary font-base lh-1 mb-1"><?= htmlspecialchars((string) (($pr['company'] ?? '') !== '' ? $pr['company'] : '-'), ENT_QUOTES, 'UTF-8') ?></h6>
                        <h6 class="fs-11 fw-semi-bold text-facebook mb-1"><?= htmlspecialchars($partnerFullName !== '' ? $partnerFullName : '-', ENT_QUOTES, 'UTF-8') ?></h6>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars((string) (($pr['phone'] ?? '') !== '' ? $pr['phone'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars((string) (($pr['address'] ?? '') !== '' ? $pr['address'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars((string) (($pr['email'] ?? '') !== '' ? $pr['email'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                      </div>
                      
                    </div>

                    <div class="row mb-2 g-2 fs-10 font-sans-serif fw-medium" style="padding-left:28px;">
                      <div class="col-4"><span class="text-600">Role:</span><br><?= htmlspecialchars((string) (($pr['role_name'] ?? '') !== '' ? $pr['role_name'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                      <div class="col-4"><span class="text-600">Type:</span><br><?= htmlspecialchars((string) ($userTypeLabels[(int) ($pr['user_type'] ?? 1)] ?? 'Standard'), ENT_QUOTES, 'UTF-8') ?></div>
                      <div class="col-4"><span class="text-600">Partner Access:</span><br><?= htmlspecialchars((string) (($pr['partner_access_type'] ?? '') !== '' ? $pr['partner_access_type'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                    </div>

                    <div class="row mb-2 g-2 fs-10 font-sans-serif fw-medium" style="padding-left:28px;">
                      <div class="col-4"><span class="text-600">Branch Access:</span><br><?= htmlspecialchars((string) (($pr['branch_access_type'] ?? '') !== '' ? $pr['branch_access_type'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                      <div class="col-8"><span class="text-600">Notes:</span><br><?= htmlspecialchars((string) (($pr['notes'] ?? '') !== '' ? $pr['notes'] : '-'), ENT_QUOTES, 'UTF-8') ?></div>
                    </div>
  ...
  <div class="col-xl-6" id="partner-tab-form">
    <div class="card h-100">
      <div class="card-header border-bottom border-200 d-flex align-items-center justify-content-between">
        <h6 class="mb-0"><?= $partnerTabFormData['id'] > 0 ? 'Update Partner' : 'Add Partner' ?></h6>
      </div>
      <div class="card-body">
        <form method="post" action="<?= htmlspecialchars($currentPath) ?>?tab=partners#partner-tab-form" class="row g-2" enctype="multipart/form-data">
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <textarea class="form-control" id="pNotes" name="notes" placeholder="Notes" style="height: 58px;"><?= htmlspecialchars($partnerTabFormData['notes']) ?></textarea>
              <label for="pNotes">Notes</label>
            </div>
          </div>
          ...
          <div class="col-md-6 col-xl-4">
            ...
            <?php if ($partnerTabFormData['logo_icon'] !== ''): ?>
              <div class="mt-2">
                <img src="<?= htmlspecialchars($appBasePath . '/' . ltrim($partnerTabFormData['logo_icon'], '/')) ?>" alt="Icon logo" style="width: 100px; height: 100px; object-fit: contain;">
              </div>
            <?php endif; ?>
          </div>
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <input class="form-control" id="pLogoMain" name="logo_main" type="file" accept="image/*" placeholder="Main Logo (300x100 px)" data-partner-main-logo-input="1" data-preview-partner-id="<?= (int) ($partnerTabFormData['id'] ?? 0) ?>" />
              <label for="pLogoMain">Main Logo (300x100 px)</label>
            </div>
            <div class="mt-2" data-partner-main-logo-form-preview-wrap="1" style="<?= $partnerTabFormData['logo_main'] !== '' ? '' : 'display:none;' ?>">
              <img data-partner-main-logo-form-preview="1" src="<?= $partnerTabFormData['logo_main'] !== '' ? htmlspecialchars($appBasePath . '/' . ltrim($partnerTabFormData['logo_main'], '/')) : '' ?>" alt="Main logo" style="max-width: 300px; height: auto;">
            </div>
          </div>
          ...
          <div class="col-md-6 col-xl-4">
            ...
            <?php if (!empty($partnerTabFormData['contact_person_img'])): ?>
              <div class="mt-2">
                <img src="<?= htmlspecialchars($appBasePath . '/' . ltrim($partnerTabFormData['contact_person_img'], '/')) ?>" alt="Contact Person IMG" style="width: 200px; height: 300px; object-fit: cover;">
              </div>
            <?php endif; ?>
          </div>
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <select class="form-select" id="pRole" name="roleId" aria-label="Role">
                ...
                <?php foreach ($activeRoles as $role): ?>
                  <option value="<?= (int) $role['role_id'] ?>"
                    <?= (int) $partnerTabFormData['roleId'] === (int) $role['role_id'] ? 'selected' : '' ?>>
                    <?= htmlspecialchars((string) $role['role_name']) ?>
                  </option>
                <?php endforeach; ?>
              </select>
            ...
            <div class="form-floating">
              <select class="form-select" id="pParent" name="parentId" aria-label="Parent Partner">
                ...
                <?php foreach ($partnerTabParentOptions as $ap): ?>
                  <option value="<?= (int) $ap['id'] ?>"
                    <?= (int) $partnerTabFormData['parentId'] === (int) $ap['id'] ? 'selected' : '' ?>>
                    #<?= (int) $ap['id'] ?> —
                    <?= htmlspecialchars(trim(($ap['firstname'] ?? '') . ' ' . ($ap['lastname'] ?? '')) ?: (string) $ap['username']) ?>
                    <?php if (!empty($ap['company'])): ?>(<?= htmlspecialchars((string) $ap['company']) ?>)<?php endif; ?>
                  </option>
          ...
          <div class="col-md-6 col-xl-4">
            <div class="border rounded px-3 py-2 h-100 d-flex align-items-center justify-content-between">
              ...
              <div class="form-check form-switch m-0">
                <input class="form-check-input" type="checkbox" id="pEnabled" name="enabled" value="1"
                       <?= (int) $partnerTabFormData['enabled'] === 1 ? 'checked' : '' ?> />
                <label class="form-check-label" for="pEnabled">Active</label>
              </div>
            </div>
          </div>
          ...
          <div class="col-12 d-flex justify-content-end gap-2 mt-2">
            ...
            <button class="btn btn-primary btn-sm" type="submit"><?= $partnerTabFormData['id'] > 0 ? 'Update' : 'Add' ?></button>
          </div>
        </form>
      </div>
    </div>
            ...
            <?php foreach ($branchListRows as $branch): ?>
              <?php
                ...
                if ($isStaticBranchCard) {
                  $branchMainLogoUrl = $appBasePath . '/assets/uploads/mycompany/main_20260425160946_24f1639f.png';
                }
              ...
              <div class="col-sm-6 col-lg-6" data-branch-card-id="<?= $branchCardId ?>">
                <div class="card position-relative rounded-4">
                  <div class="bg-holder bg-card rounded-4" style="background-image:url(<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/assets/img/icons/spot-illustrations/<?= $isStaticBranchCard ? 'corner-2' : 'corner-1' ?>.png);"></div>
                  <div class="card-body p-3 pt-4 pt-xxl-4">
                    <div class="d-flex align-items-center justify-content-between mb-2 gap-2">
                      <a class="btn btn-link p-0 position-relative z-2 d-flex align-items-center gap-1" href="<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/app/administration/my-company.php?tab=branches&branch_edit_id=<?= $branchCardId ?>#branch-tab-form" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit" aria-label="Edit Branch">
                        <span class="fas fa-edit text-500"></span>
                        <small class="badge rounded <?= $branchStatus === 1 ? 'badge-subtle-success' : 'badge-subtle-danger' ?>">
                          <?= $branchStatus === 1 ? 'Active' : 'Inactive' ?>
                        </small>
                      </a>
                      <div class="d-flex justify-content-end">
                        <?php if ($branchMainLogoUrl !== ''): ?>
                          <img src="<?= htmlspecialchars($branchMainLogoUrl, ENT_QUOTES, 'UTF-8') ?>" alt="Main logo" height="72" style="max-width: 240px; object-fit: contain;">
                        <?php else: ?>
                          <span class="fas fa-building text-primary fs-4"></span>
                        <?php endif; ?>
                      </div>
                    ...
                    <div class="row align-items-stretch">
                      ...
                      <div class="col-6 text-start">
                        <div class="fs-10 text-600 mb-1"><strong>Contact Person</strong></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars($branchContactName !== '' ? $branchContactName : '-', ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars($branchContactPhone !== '' ? $branchContactPhone : '-', ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars($branchContactAltPhone !== '' ? $branchContactAltPhone : '-', ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars($branchContactEmail !== '' ? $branchContactEmail : '-', ENT_QUOTES, 'UTF-8') ?></div>
                      </div>
                      <div class="col-6 text-end">
                        <h6 class="text-primary font-base lh-1 mb-1"><?= htmlspecialchars($branchTitle !== '' ? $branchTitle : '-', ENT_QUOTES, 'UTF-8') ?></h6>
                        <h6 class="fs-11 fw-semi-bold text-facebook mb-1"><?= htmlspecialchars($partnerTitle !== '' ? $partnerTitle : '-', ENT_QUOTES, 'UTF-8') ?></h6>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars($displayPhone !== '' ? $displayPhone : '-', ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars($displayAddress !== '' ? $displayAddress : '-', ENT_QUOTES, 'UTF-8') ?></div>
                        <div class="fs-10 text-600 mb-0"><?= htmlspecialchars($displayEmail !== '' ? $displayEmail : '-', ENT_QUOTES, 'UTF-8') ?></div>
                      </div>
                    </div>

                    <div class="row mb-2 g-2 fs-10 font-sans-serif fw-medium" style="padding-left:28px;">
                      <div class="col-4"><span class="text-600">Role:</span><br><?= htmlspecialchars($displayRole, ENT_QUOTES, 'UTF-8') ?></div>
                      <div class="col-4"><span class="text-600">Type:</span><br><?= htmlspecialchars($displayType, ENT_QUOTES, 'UTF-8') ?></div>
                      <div class="col-4"><span class="text-600">Partner Access:</span><br><?= htmlspecialchars($displayPartnerAccess, ENT_QUOTES, 'UTF-8') ?></div>
                    </div>

                    <div class="row mb-2 g-2 fs-10 font-sans-serif fw-medium" style="padding-left:28px;">
                      <div class="col-4"><span class="text-600">Branch Access:</span><br><?= htmlspecialchars($displayBranchAccess, ENT_QUOTES, 'UTF-8') ?></div>
                      <div class="col-8"><span class="text-600">Notes:</span><br><?= htmlspecialchars($displayNotes, ENT_QUOTES, 'UTF-8') ?><?php if ($displayLocation !== '-'): ?> | Location: <?= htmlspecialchars($displayLocation, ENT_QUOTES, 'UTF-8') ?><?php endif; ?></div>
                    </div>
  ...
  <div class="col-xl-4" id="branch-tab-form">
    <div class="card h-100">
      ...
      <div class="card-body">
        <form class="row g-2" action="<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/app/administration/my-company.php?tab=branches#branch-tab-form" method="post">
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <input class="form-control" id="branchName" name="branch_name" type="text" placeholder="Enter branch name" required value="<?= htmlspecialchars((string) ($branchFormData['branch_name'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" />
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <?php if ($isStaticBranchForm): ?>
                <input class="form-control" id="branchDisplayCompany" name="display_company" type="text" placeholder="Manual Company Name" value="<?= htmlspecialchars((string) (($branchFormData['display_company'] ?? '') !== '' ? $branchFormData['display_company'] : 'Bestnet'), ENT_QUOTES, 'UTF-8') ?>" />
                <label for="branchDisplayCompany">Company Name (Manual)</label>
              <?php else: ?>
                <select class="form-select" id="branchPartner" name="partner_id" aria-label="Partner">
                  <option value="" disabled <?= (int) ($branchFormData['partner_id'] ?? 0) === 0 ? 'selected' : '' ?>>Select Partner</option>
                  <?php foreach ($branchPartnerOptions as $branchPartner): ?>
                    <?php
                      $branchPartnerName = trim(((string) ($branchPartner['firstname'] ?? '')) . ' ' . ((string) ($branchPartner['lastname'] ?? '')));
                      $branchPartnerDisplay = $branchPartnerName !== '' ? $branchPartnerName : (string) ($branchPartner['username'] ?? 'Partner');
                      if (!empty($branchPartner['company'])) {
                        $branchPartnerDisplay .= ' (' . (string) $branchPartner['company'] . ')';
                      }
                      $branchPartnerOptionId = (int) ($branchPartner['id'] ?? 0);
                    ?>
                    <option value="<?= $branchPartnerOptionId ?>" <?= (int) ($branchFormData['partner_id'] ?? 0) === $branchPartnerOptionId ? 'selected' : '' ?>><?= htmlspecialchars($branchPartnerDisplay, ENT_QUOTES, 'UTF-8') ?></option>
                  <?php endforeach; ?>
                </select>
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <select class="form-select" id="branchLocation" name="location_id" aria-label="Inventory Location (Sub area)">
                <option value="" disabled <?= (int) ($branchFormData['location_id'] ?? 0) === 0 ? 'selected' : '' ?>>Select Location List (Sub area)</option>
                <?php foreach ($branchLocationOptions as $location): ?>
                  ...
                  ?>
                  <option value="<?= $locationOptionId ?>" <?= (int) ($branchFormData['location_id'] ?? 0) === $locationOptionId ? 'selected' : '' ?>><?= htmlspecialchars($locationLabel, ENT_QUOTES, 'UTF-8') ?></option>
                <?php endforeach; ?>
              </select>
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <input class="form-control" id="branchEmail" name="email" type="email" placeholder="Enter email" required value="<?= htmlspecialchars((string) ($branchFormData['email'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" />
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <input class="form-control" id="branchMobile" name="mobile" type="text" placeholder="Enter mobile number" required value="<?= htmlspecialchars((string) ($branchFormData['mobile'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" />
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <input class="form-control" id="branchContactPersonName" name="contact_person_name" type="text" placeholder="Contact Person Name" value="<?= htmlspecialchars((string) ($branchFormData['contact_person_name'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" />
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <input class="form-control" id="branchContactPersonPhone" name="contact_person_phone" type="text" placeholder="Contact Person Phone" value="<?= htmlspecialchars((string) ($branchFormData['contact_person_phone'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" />
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <input class="form-control" id="branchContactPersonAltPhone" name="contact_person_alt_phone" type="text" placeholder="Alternative Phone" value="<?= htmlspecialchars((string) ($branchFormData['contact_person_alt_phone'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" />
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <input class="form-control" id="branchContactPersonEmail" name="contact_person_email" type="email" placeholder="Contact Person Email" value="<?= htmlspecialchars((string) ($branchFormData['contact_person_email'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" />
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <textarea class="form-control" id="branchAddress" name="address" placeholder="Enter address" style="height: 58px;" required><?= htmlspecialchars((string) ($branchFormData['address'] ?? ''), ENT_QUOTES, 'UTF-8') ?></textarea>
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <select class="form-select" id="branchType" name="branch_type" aria-label="Type">
                <option value="" disabled <?= (string) ($branchFormData['branch_type'] ?? '') === '' ? 'selected' : '' ?>>Select Type</option>
                ...
                <option value="local" <?= (string) ($branchFormData['branch_type'] ?? '') === 'local' ? 'selected' : '' ?>>Local</option>
          ...
          <div class="col-md-6 col-xl-4">
            <div class="form-floating">
              <input class="form-control" id="branchRatio" name="ratio" type="number" min="0" max="100" placeholder="e.g. 30" value="<?= htmlspecialchars((string) ($branchFormData['ratio'] ?? '0'), ENT_QUOTES, 'UTF-8') ?>" />
          ...
          <div class="col-12 d-flex justify-content-end gap-2 mt-2">
            <a class="btn btn-falcon-default btn-sm" href="<?= htmlspecialchars($appBasePath, ENT_QUOTES, 'UTF-8') ?>/app/administration/my-company.php?tab=branches#branch-tab-form">Reset</a>
            <button class="btn btn-primary btn-sm" type="submit">
